...
<div style="width:800px; height:330px; background:#00CCFF; margin:0px auto 0px auto"><center><h1 style="table-layout:0; color:#663333;">TICKET REPORT</h1></center>

<?php include('connection.php'); ?>

<body>
               <center> <form method="post" action="ticketsearch.php">
...
<div style=" max-height:200px; max-width:600px; overflow:scroll">
              ...
              <center>  <table cellpadding="2" cellspacing="2" border="2" bordercolor="#000000" class="table table-striped table-bordered" >
                    ...
                    <thead >
                         <tr> <th>id</th><th>date</th><th>names</th><th>time</th><th>Trip</th><th>price</th><th>Plate Number</th><th>UPDATE</th></tr>
                    </thead>
                    <tbody>
                        <?php
                        $query =mysql_query("select t.t_id,t.date,t.tr_names,t.time,t.destination,t.price,c.plac from ticket as t,gar as c where t.cr_id=c.cr_id")or die(mysql_error());
                ...
                echo '<td>' . $row['t_id'] . '</td>';
				echo '<td>' . $row['date'] . '</td>';
				echo '<td>' . $row['tr_names'] . '</td>';
				echo '<td>' . $row['time'] . '</td>';
				echo '<td>' . $row['destination'] . '</td>';
				echo '<td>' . $row['price'] . '</td>';
				echo '<td>' . $row['plac'] . '</td>';
	 echo"<td ><a href ='ticketup.php?t_id=$id'>";?><button type="button" name="Submit" onClick="return confirm('You Are Going to Update Record!!')">update</button><?php print"</a></td>";
